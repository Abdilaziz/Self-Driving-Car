...
UKF::UKF() {
  ...
  std_a_ = 2; // Tuning Param

  // Process noise standard deviation yaw acceleration in rad/s^2
  std_yawdd_ = 0.3; // Tuning Param
  ...
  std_radrd_ = 0.3;
	...
	lambda_ = 3 - n_aug_; // n_x_ or n_aug_
...
void UKF::ProcessMeasurement(MeasurementPackage meas_package) {
	...
	if (!is_initialized_) {
		...
		if (meas_package.sensor_type_ == MeasurementPackage::RADAR) {
			...
			double vx = ro_dot*cphi;
			double vy = ro_dot*sphi;

			double v = sqrt( vx*vx + vy*vy);

			// px, py, v, yaw, yaw_dot
			x_ << ro*cphi, ro*sphi, v, 0, 0;
...
void UKF::Prediction(double delta_t) {
  ...
  */
	...
	*/
	...
	for (int i = 0; i < 2 * n_aug_ + 1; i++) 
	{
		...
		if (fabs(yaw_dot) > 0.001) {
			...
			py_pred = py + (v / yaw_dot)*( cos(yaw) - cos(yaw + yaw_dot*delta_t) ) + (0.5* (delta_t*delta_t)*sin(yaw)*nu_accel);
		}
		else {
	...
	*/
	...
	x_.fill(0.0); // already a part of sigma points so we can reset it
	for (int i = 0; i < 2 * n_aug_ + 1; i++) {  //iterate over sigma points
		x_ = x_ + weights_(i) * Xsig_pred_.col(i);
	}

	// Calculate Covariance Matrix of State
	P_.fill(0.0); // already a part of sigma points so we can reset it
	for (int i = 0; i < 2 * n_aug_ + 1; i++) {
		// state difference
